...
from ..providers import Cryptocompare
...
def import_all(symbol: str):
    try:
        ...
        ohlc_inputs = Cryptocompare().get_ohlc_full(fsym=symbol)
        log.info(f"Obtained {len(ohlc_inputs)} ohlc inputs from Cryptocompare API")
...
def import_last_delta(symbol: str):
    try:
        ...
        if is_updated:
            ...
            return False
        delta = Cryptocompare().get_ohlc_limit(fsym=symbol, limit=1)
